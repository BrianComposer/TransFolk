...
from transfolk_classifier.sequential_classifier import (
    load_sequence_model_and_evaluate,
    train_and_save_sequence_model,
)
...
def train_and_save_model(
    ...
    random_state: int = 42,
    tokenization_json_path: str = None,
) -> str:

    os.makedirs(out_dir, exist_ok=True)

    # Rama secuencial MoMeT: no extrae features tabulares y conserva intacto
    # el flujo clasico para algorithm_id 1..9.
    if ALGORITHM_NAME.get(algorithm_id) == "cnn1d_momet":
        return train_and_save_sequence_model(
            df_split=df_split,
...
def load_model_and_evaluate(
    ...
    # Rama secuencial MoMeT: produce el mismo CSV de evaluacion que el
    # pipeline clasico, para que compare/curves/final funcionen sin cambios.
    if os.path.exists(os.path.join(model_dir, "model.pt")):
        return load_sequence_model_and_evaluate(
            model_dir=model_dir,
            df_split=df_split,
